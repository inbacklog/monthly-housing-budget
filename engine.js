...
function validate(input){
 ...
 for(const arr of ['lines','transactions']){
  ...
  const seen=new Set();p[arr]=input[arr].map(l=>{
   const o={id:text(l.id,80),type:text(l.type,20),label:text(l.label,160),note:text(l.note??'',240),category:text(l.category,80),member:text(l.member||'',80),amount:num(l.amount),funding:l.funding||'cash'};
...
function forecast(p,options={}){
 const settings={...p.settings,...options};let cash=settings.openingCash,saved=0,invested=0,investmentValue=settings.investmentStart;const rows=[];
 const investmentFactor=Math.pow(1+settings.investmentReturn/100,1/12);
 for(let i=0;i<settings.horizon;i++){
  ...
  const alloc=allocation(s,settings);cash+=alloc.remaining;saved+=s.saving+alloc.extraSaving;invested+=alloc.investment;investmentValue=investmentValue*investmentFactor+alloc.investment;
  const investmentContributed=settings.investmentStart+invested,investmentGrowth=investmentValue-investmentContributed;
  rows.push({month,income:s.income,expenses:s.expenses,savings:s.saving+alloc.extraSaving,investment:alloc.investment,remaining:alloc.remaining,cash,totalSaved:saved,totalInvested:invested,investmentContributed,investmentValue,investmentGrowth,unfunded:Math.max(0,-s.unassigned)});
 }
 return rows;
}
...
const CSV_COLS_LEGACY=['kind','date','type','description','category','amount','frequency','member','essential','subscription','funding'];
const CSV_COLS=[...CSV_COLS_LEGACY,'note'];
function importCSV(input,base){const rows=parseCSV(input);if(!rows.length)throw Error('Empty CSV');const headers=rows.shift().map(x=>x.trim().toLowerCase());const schema=headers.length===CSV_COLS.length&&CSV_COLS.every((h,i)=>headers[i]===h)?CSV_COLS:headers.length===CSV_COLS_LEGACY.length&&CSV_COLS_LEGACY.every((h,i)=>headers[i]===h)?CSV_COLS_LEGACY:null;if(!schema)throw Error('Use the Homeflow CSV template headers / χρησιμοποίησε το πρότυπο');
 ...
 rows.forEach((r,i)=>{try{
  if(schema===CSV_COLS&&r.length===CSV_COLS.length-1)r.push('');if(r.length!==headers.length)throw Error('Wrong column count');const d=Object.fromEntries(headers.map((h,j)=>[h,(r[j]??'').trim()]));
  ...
  const item={id:id(),type:d.type,label:d.description,note:d.note||'',category:d.category,amount:Number(d.amount.replace(',','.')),member,funding:d.funding||'cash'};
...
function exportCSV(p){const rows=[CSV_COLS];for(const kind of ['plan','actual'])for(const l of p[kind==='plan'?'lines':'transactions'].filter(l=>kind==='actual'||l.active))rows.push([kind,kind==='plan'?(l.month||''):l.date,l.type,l.label,l.category,l.amount,kind==='plan'?l.frequency:'',l.member?(p.members.find(m=>m.id===l.member)?.name||'Member '+(p.members.findIndex(m=>m.id===l.member)+1)):'',l.essential||false,l.subscription||false,l.funding,l.note||'']);return '\uFEFF'+rows.map(r=>r.map(csvEscape).join(',')).join('\r\n');}
function anonymize(p,withActual=false){const c=clone(p);c.title='';c.members=c.members.map((m,i)=>({...m,name:'Member '+(i+1)}));c.lines=c.lines.map((l,i)=>({...l,label:l.category+' '+(i+1),note:''}));c.transactions=withActual?c.transactions.map((l,i)=>({...l,label:l.category+' '+(i+1),note:''})):[];if(!withActual)c.closedMonths=[];return c;}
function demo(month=monthNow()){
 ...
 p.lines=defs.map((a,i)=>({id:'demo'+i,type:a[0],label:a[1],note:'',category:a[2],amount:a[3],frequency:a[4],member:a[5],essential:a[6],subscription:a[2]==='subscriptions',active:true,funding:'cash',month:''}));p.settings.openingCash=1500;p.settings.emergencyFund=1000;return p;
}
